"""
models/v20_c2f_eca_dilated/train.py
====================================
Train YOLOv8n with Dilated (d=2) Receptive Field ECA Bottlenecks (C2f_ECA_Dilated).
"""
...
VARIANT_ID = "v20"
VARIANT_NAME = "v20_c2f_eca_dilated"
...
def train(epochs: int = 50, batch: int = 16, imgsz: int = 640) -> None:
    ...
    log_run(
        model_id=VARIANT_ID,
        model_name=f"{VARIANT_NAME} (Dilated d=2 Receptive Field + ECA)",
        ...
        notes="Dilated 3x3 convolution (d=2, p=2) inside ECA bottlenecks with 100% yolov8n.pt transfer",
    )

    # Log official eval run
    log_run(
        model_id=f"{VARIANT_ID}_eval",
        model_name=f"{VARIANT_NAME} (Dilated ECA Bottleneck) [Official Test]",
        folder=f"models/{VARIANT_NAME}",
        params_M=round(n_params, 2),
        gflops=8.1,
        epochs=epochs,
    ...
    print(f"\n[v20_c2f_eca_dilated] Val mAP50: {mAP50_val*100:.2f}% | Test mAP50: {mAP50_test*100:.2f}%")
