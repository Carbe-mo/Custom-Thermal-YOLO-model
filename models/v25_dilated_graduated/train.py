"""
models/v25_dilated_graduated/train.py
====================================
v25: Graduated Dilation ECA Backbone.

Stage-selective dilation creates a receptive field pyramid:
  P2 (d=1): standard 3x3 — fine boundary detection for small objects
  P3 (d=1): standard 3x3 — small person detection
  P4 (d=2): 5×5 effective RF — medium objects (bags)
  P5 (d=3): 7×7 effective RF — large objects (trolleys, scenes)

All kernel shapes remain 3×3 → identical to yolov8n.pt weight tensors → full transfer.
Single variable vs. v20: graduated d per stage instead of uniform d=2.
"""
...
from common.modules.c2f_eca_graduated import register_c2f_eca_graduated
...
VARIANT_ID = "v25"
VARIANT_NAME = "v25_dilated_graduated"
...
def train(epochs: int = 50, batch: int = 16, imgsz: int = 640) -> None:
    ...
    log_run(
        model_id=VARIANT_ID,
        model_name=f"{VARIANT_NAME} (Graduated Dilation d=1/1/2/3 per stage + ECA)",
        ...
        notes="graduated dilation: P2/P3 d=1, P4 d=2, P5 d=3; full pretrained transfer",
    )

    log_run(
        model_id=f"{VARIANT_ID}_eval",
        model_name=f"{VARIANT_NAME} (Graduated Dilation ECA) [Official Test]",
        folder=f"models/{VARIANT_NAME}",
        params_M=round(n_params, 2),
        gflops=8.1,
        epochs=epochs,
