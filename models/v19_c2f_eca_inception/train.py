"""
models/v19_c2f_eca_inception/train.py
======================================
Train YOLOv8n with Inception Strip Convolutions + ECA Attention (C2f_ECA_Inception).
"""
...
from common.modules.c2f_eca_inception import register_c2f_eca_inception
...
VARIANT_ID = "v19"
VARIANT_NAME = "v19_c2f_eca_inception"
...
def train(epochs: int = 50, batch: int = 16, imgsz: int = 640) -> None:
    ...
    log_run(
        model_id=VARIANT_ID,
        model_name=f"{VARIANT_NAME} (Inception Strip Convs + ECA)",
        ...
        notes="Multi-scale asymmetric strip convolutions (1x5 + 5x1 + 3x3 + 1x1) with ECA channel attention in bottlenecks",
    )

    # Log official eval run
    log_run(
        model_id=f"{VARIANT_ID}_eval",
        model_name=f"{VARIANT_NAME} (Inception ECA Bottleneck) [Official Test]",
        folder=f"models/{VARIANT_NAME}",
        params_M=round(n_params, 2),
        gflops=8.3,
        epochs=epochs,
    ...
    print(f"\n[v19_c2f_eca_inception] Val mAP50: {mAP50_val*100:.2f}% | Test mAP50: {mAP50_test*100:.2f}%")
