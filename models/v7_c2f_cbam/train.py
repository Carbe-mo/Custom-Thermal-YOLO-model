"""
models/v7_c2f_cbam/train.py
===========================
Train YOLOv8n with C2f_CBAM_Bottle (CBAM attention inside Bottlenecks) with pretrained weights.
"""
...
def train(epochs: int = 50, batch: int = 16, imgsz: int = 640) -> None:
    ...
    log_run(
        model_id=VARIANT_ID,
        model_name=f"{VARIANT_NAME} (YOLOv8n + CBAM inside Bottlenecks)",
        ...
        notes="CBAM channel+spatial attention inside residual bottleneck branch with pretrained transfer",
    )

    # Log official eval run
    log_run(
        model_id=f"{VARIANT_ID}_eval",
        model_name=f"{VARIANT_NAME} (CBAM in Bottleneck) [Official Test]",
        folder=f"models/{VARIANT_NAME}",
        params_M=round(n_params, 2),
        gflops=8.2,
        epochs=epochs,
    ...
    print(f"\n[v7_c2f_cbam] Val mAP50: {mAP50_val*100:.2f}% | Test mAP50: {mAP50_test*100:.2f}%")
