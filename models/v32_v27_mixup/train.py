"""
models/v32_v27_mixup/train.py
===============================
Train v21 Hybrid Dilated ECA with gentle mixup=0.1 augmentation + 75-epoch Cosine Annealing.
"""
...
def train(epochs: int = 75, batch: int = 16, imgsz: int = 640) -> None:
    ...
    log_run(
        model_id=VARIANT_ID,
        model_name=f"{VARIANT_NAME} (v27 Hybrid Dilated + mixup=0.1 + Cosine 75ep)",
        ...
        notes="v21 Hybrid Dilated + mixup=0.1 + 75 epochs Cosine LR schedule",
    )

    log_run(
        model_id=f"{VARIANT_ID}_eval",
        model_name=f"{VARIANT_NAME} (Mixup + Hybrid Dilated) [Official Test]",
        folder=f"models/{VARIANT_NAME}",
        params_M=round(n_params, 2),
        gflops=8.1,
        epochs=epochs,
