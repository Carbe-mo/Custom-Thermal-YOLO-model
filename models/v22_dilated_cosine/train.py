"""
models/v22_dilated_cosine/train.py
====================================
v20 architecture (all-dilated ECA bottlenecks) trained for 100 epochs
with cosine LR decay (cos_lr=True, lrf=0.01).

Single variable vs. v20: training schedule only.
Hypothesis: dilated convolutions have sparse sampling grids and need more
gradient steps to align their weights with the COCO-pretrained 3x3 kernels.
Cosine annealing provides a smoother final convergence phase.
"""
...
VARIANT_ID = "v22"
VARIANT_NAME = "v22_dilated_cosine"
...
def train(epochs: int = 100, batch: int = 16, imgsz: int = 640) -> None:
    ...
    model.train(
        ...
        cos_lr=True,          # Cosine LR annealing — key change vs v20
        lrf=0.01,             # Final LR ratio
        close_mosaic=20,      # Give 20 extra epochs without mosaic at end
        verbose=True,
    )
    ...
    log_run(
        model_id=VARIANT_ID,
        model_name=f"{VARIANT_NAME} (Dilated ECA + 100ep Cosine LR)",
        ...
        notes="v20 arch + 100 epochs + cos_lr=True; tests if dilated kernels need longer convergence",
    )

    log_run(
        model_id=f"{VARIANT_ID}_eval",
        model_name=f"{VARIANT_NAME} (Dilated ECA 100ep Cosine) [Official Test]",
        folder=f"models/{VARIANT_NAME}",
        params_M=round(n_params, 2),
        gflops=8.1,
        epochs=epochs,
