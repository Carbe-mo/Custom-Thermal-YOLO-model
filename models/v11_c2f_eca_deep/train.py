"""
models/v11_c2f_eca_deep/train.py
================================
Train YOLOv8n with deepened C2f_ECA backbone stages (n=[2, 3, 3, 2]).
"""
...
def train(epochs: int = 50, batch: int = 16, imgsz: int = 640) -> None:
    ...
    log_run(
        model_id=VARIANT_ID,
        model_name=f"{VARIANT_NAME} (YOLOv8n + Deepened C2f_ECA Backbone)",
        ...
        notes="Deepened C2f_ECA backbone bottleneck depth n=[2, 3, 3, 2] with pretrained transfer",
    )

    # Log official eval run
    log_run(
        model_id=f"{VARIANT_ID}_eval",
        model_name=f"{VARIANT_NAME} (Deepened C2f_ECA) [Official Test]",
        folder=f"models/{VARIANT_NAME}",
        params_M=round(n_params, 2),
        gflops=9.3,
        epochs=epochs,
    ...
    print(f"\n[v11_c2f_eca_deep] Val mAP50: {mAP50_val*100:.2f}% | Test mAP50: {mAP50_test*100:.2f}%")
