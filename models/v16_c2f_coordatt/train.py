"""
models/v16_c2f_coordatt/train.py
================================
Train YOLOv8n with C2f_CoordAtt (Coordinate Attention inside Backbone Bottlenecks).
"""
...
def train(epochs: int = 50, batch: int = 16, imgsz: int = 640) -> None:
    ...
    log_run(
        model_id=VARIANT_ID,
        model_name=f"{VARIANT_NAME} (YOLOv8n + Coordinate Attention in Bottlenecks)",
        ...
        notes="Coordinate Attention (1D horizontal + 1D vertical spatial pooling) in backbone bottlenecks with pretrained weights",
    )

    # Log official eval run
    log_run(
        model_id=f"{VARIANT_ID}_eval",
        model_name=f"{VARIANT_NAME} (Coordinate Attention in Bottleneck) [Official Test]",
        folder=f"models/{VARIANT_NAME}",
        params_M=round(n_params, 2),
        gflops=8.2,
        epochs=epochs,
    ...
    print(f"\n[v16_c2f_coordatt] Val mAP50: {mAP50_val*100:.2f}% | Test mAP50: {mAP50_test*100:.2f}%")
