"""
models/v29_v21_p3p4_dilated/train.py
======================================
Train YOLOv8n with Dilation at P3 and P4 (d=2) and standard C2f_ECA at P2 and P5.
"""
...
VARIANT_ID = "v29"
VARIANT_NAME = "v29_v21_p3p4_dilated"
...
def train(epochs: int = 50, batch: int = 16, imgsz: int = 640) -> None:
    ...
    log_run(
        model_id=VARIANT_ID,
        model_name=f"{VARIANT_NAME} (Dilation at P3 and P4 + ECA)",
        ...
        notes="Dilation d=2 at P3 and P4; standard ECA at P2 and P5",
    )

    log_run(
        model_id=f"{VARIANT_ID}_eval",
        model_name=f"{VARIANT_NAME} (P3+P4 Dilated ECA) [Official Test]",
        folder=f"models/{VARIANT_NAME}",
        params_M=round(n_params, 2),
        gflops=8.1,
        epochs=epochs,
