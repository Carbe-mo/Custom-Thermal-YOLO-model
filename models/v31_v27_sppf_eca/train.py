"""
models/v31_v27_sppf_eca/train.py
==================================
Train v21 Hybrid Dilated ECA with SPPF_ECA layer (layer 9) + 75-epoch Cosine Annealing.
Hypothesis: Adding ECA attention right after SPPF multi-scale feature pooling calibrates
the concatenated multi-scale channels before they propagate into the PANet neck.
"""
...
from common.modules.sppf_eca import register_sppf_eca
...
def train(epochs: int = 75, batch: int = 16, imgsz: int = 640) -> None:
    ...
    log_run(
        model_id=VARIANT_ID,
        model_name=f"{VARIANT_NAME} (v27 Hybrid Dilated + SPPF_ECA + Cosine 75ep)",
        ...
        notes="v21 Hybrid Dilated + SPPF_ECA layer 9 + 75 epochs Cosine LR schedule",
    )

    log_run(
        model_id=f"{VARIANT_ID}_eval",
        model_name=f"{VARIANT_NAME} (SPPF_ECA + Hybrid Dilated) [Official Test]",
        folder=f"models/{VARIANT_NAME}",
        params_M=round(n_params, 2),
        gflops=8.1,
        epochs=epochs,
