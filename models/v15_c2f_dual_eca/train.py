"""
models/v15_c2f_dual_eca/train.py
================================
Train YOLOv8n with C2f_DualECA (Dual ECA Channel Attention inside Backbone Bottlenecks).
"""
...
def train(epochs: int = 50, batch: int = 16, imgsz: int = 640) -> None:
    ...
    log_run(
        model_id=VARIANT_ID,
        model_name=f"{VARIANT_NAME} (YOLOv8n + Dual ECA in Bottlenecks)",
        ...
        notes="Dual ECA channel attention (post-cv1 and post-cv2) in backbone bottlenecks with pretrained weights",
    )

    # Log official eval run
    log_run(
        model_id=f"{VARIANT_ID}_eval",
        model_name=f"{VARIANT_NAME} (Dual ECA in Bottleneck) [Official Test]",
        folder=f"models/{VARIANT_NAME}",
        params_M=round(n_params, 2),
        gflops=8.2,
        epochs=epochs,
    ...
    print(f"\n[v15_c2f_dual_eca] Val mAP50: {mAP50_val*100:.2f}% | Test mAP50: {mAP50_test*100:.2f}%")
