"""
models/v8_c2f_pconv/train.py
============================
Train YOLOv8n with C2f_PConv (FasterNet Partial Convolutions in Bottlenecks).
"""
...
def train(epochs: int = 50, batch: int = 16, imgsz: int = 640) -> None:
    ...
    log_run(
        model_id=VARIANT_ID,
        model_name=f"{VARIANT_NAME} (YOLOv8n + FasterNet PConv in Bottlenecks)",
        ...
        notes="FasterNet 1/4 partial convolution inside backbone bottlenecks with pretrained weights",
    )

    # Log official eval run
    log_run(
        model_id=f"{VARIANT_ID}_eval",
        model_name=f"{VARIANT_NAME} (FasterNet PConv in Bottleneck) [Official Test]",
        folder=f"models/{VARIANT_NAME}",
        params_M=round(n_params, 2),
        gflops=7.0,
        epochs=epochs,
    ...
    print(f"\n[v8_c2f_pconv] Val mAP50: {mAP50_val*100:.2f}% | Test mAP50: {mAP50_test*100:.2f}%")
