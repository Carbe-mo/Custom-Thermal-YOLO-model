"""
models/v9_c2f_eca_all/train.py
==============================
Train YOLOv8n with C2f_ECA across both Backbone and Neck/Head stages.
"""
...
def train(epochs: int = 50, batch: int = 16, imgsz: int = 640) -> None:
    ...
    log_run(
        model_id=VARIANT_ID,
        model_name=f"{VARIANT_NAME} (YOLOv8n + Full-Network C2f_ECA)",
        ...
        notes="ECA channel attention in all 8 C2f blocks (Backbone + PANet Neck) with pretrained transfer",
    )

    # Log official eval run
    log_run(
        model_id=f"{VARIANT_ID}_eval",
        model_name=f"{VARIANT_NAME} (Full-Network C2f_ECA) [Official Test]",
        folder=f"models/{VARIANT_NAME}",
        params_M=round(n_params, 2),
        gflops=8.2,
        epochs=epochs,
    ...
    print(f"\n[v9_c2f_eca_all] Val mAP50: {mAP50_val*100:.2f}% | Test mAP50: {mAP50_test*100:.2f}%")
