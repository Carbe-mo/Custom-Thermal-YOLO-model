"""
models/v10_c2f_eca_adaptive/train.py
====================================
Train YOLOv8n with C2f_ECA_Adaptive (Adaptive Kernel ECA in Backbone Bottlenecks).
"""
...
VARIANT_ID = "v10"
VARIANT_NAME = "v10_c2f_eca_adaptive"
...
def train(epochs: int = 50, batch: int = 16, imgsz: int = 640) -> None:
    ...
    log_run(
        model_id=VARIANT_ID,
        model_name=f"{VARIANT_NAME} (YOLOv8n + Adaptive Kernel ECA)",
        ...
        notes="Adaptive ECA kernel k=3 (P2/P3) and k=5 (P4/P5) in backbone bottlenecks with pretrained transfer",
    )

    # Log official eval run
    log_run(
        model_id=f"{VARIANT_ID}_eval",
        model_name=f"{VARIANT_NAME} (Adaptive ECA) [Official Test]",
        folder=f"models/{VARIANT_NAME}",
        params_M=round(n_params, 2),
        gflops=8.2,
        epochs=epochs,
    ...
    print(f"\n[v10_c2f_eca_adaptive] Val mAP50: {mAP50_val*100:.2f}% | Test mAP50: {mAP50_test*100:.2f}%")
